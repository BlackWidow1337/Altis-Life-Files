...
switch (_filter) do
{
	...
	case 0:
	{
		...
		{
		    _ret pushBack ["U_NikosAgedBody",nil,150];
			_ret pushBack ["U_Marshal",nil,150];
		};
	};
	...
	case 1:
	{
		if(__GETC__(life_coplevel) > 1) then
		{
			_ret pushBack ["H_Beret_blk_POLICE",nil,200];
			_ret pushBack ["H_CrewHelmetHeli_B","Gas Mask",nil,70];
		};
