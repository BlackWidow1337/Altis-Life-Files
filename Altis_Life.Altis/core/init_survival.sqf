...
[] spawn
{
	...
	while{true} do
	{
		...
		_load = round(_cfg / 8);
                if (backpack player == "B_Carryall_cbr") then { _load = 130; }; //add this line as many times as you want until you've covered all the desired backpacks
                if (backpack player == "B_Bergen_sgg") then { _load = 110; };
